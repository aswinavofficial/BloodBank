...
class User {
   ...
   public function  request($reg_no,$email_id,$ser_type,$location,$brief,$detailed)
   {
	...
	public function activity_logv($reg_no) {
        $sql = "select t.trans_id,t.req_id,d.org_name as d_org_name,s.org_name,r.location,s.type,r.ser_type,t.open_date,t.status from donor d,seeker s,req r,donor_trans t,vendor v where s.reg_no=t.regs and r.id=t.req_id and status='WAITING' and r.ser_type=v.ser_type and v.reg_no='$reg_no' group by t.trans_id ";
        return $this->dbObj->ExecuteQuery($sql, 1);
    }
